            ...
            <div>
                ...
                <AliceCarousel autoPlay={true} autoPlayInterval={1050} animationDuration={800} mouseTracking infinite={true} disableButtonsControls={true} disableDotsControls={true} responsive={responsive}>
                    {topCoins.map((item, index) => {
                        return (
                            <div className='mx-7'>
                                <div key={item.id + index} className="w-80 h-44 rounded-md flex flex-col border border-slate-800">
                                    ...
                                    <div className="flex text-sm items-center justify-between pl-3 pr-8 mb-3">
                                        <p className='mt-2 text-center'><span className='text-white font-semibold'>price:</span> $ {item.current_price.toLocaleString()} </p>
                                        <p className={item.price_change_percentage_24h > 0 ? "text-green-600" : "text-red-600"}> <span className='text-white font-semibold'>change:</span> {item.price_change_percentage_24h.toFixed(2)}%</p>
                                    </div>
                                    ...
                                    <div className='flex items-center text-sm justify-between gap-4 pr-0 pl-0'>
                                        ...
                                        <div className='flex text-sm items-center gap-1 pl-1 pr-2'>
